document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderHistory = (history, searchTerm) => {
        ...
        if (history.length > 0) { // Luôn hiển thị nếu có lịch sử
        ...
        history.forEach(tx => {
            ...
            if (isDeparted) {
                const fee = tx.fee || 0;
                const exitTime = new Date(tx.exit_time);
                const entryTime = new Date(tx.entry_time);

                // Tạo các tham số cho URL của biên lai
                const params = new URLSearchParams({
                    ...
                    uniqueID: tx.unique_id, plate: tx.plate, vehicleType: 'Xe máy',
                    entryTimeDisplay: formatDateTime(tx.entry_time),
                    exitTimeDisplay: formatDateTime(tx.exit_time),
                    duration: calculateDuration(tx.entry_time, tx.exit_time),
                    feeDisplay: fee.toLocaleString('vi-VN') + 'đ',
                    paymentMethod: tx.payment_method || '--',
                });

                receiptActionHtml = `<div class="receipt-action"><a href="receipt_viewer.html?${params.toString()}" target="_blank" class="btn-print">Xem biên lai</a></div>`;
            }
    ...
    const downloadAllReceipts = () => {
        const departedHistory = currentHistory.filter(tx => tx.status !== 'Đang gửi');
        if (departedHistory.length === 0) {
            showMessage('Không có biên lai nào để tải (chỉ các lần gửi xe đã hoàn tất mới có biên lai).', true);
            return;
        }
        ...
        allReceiptsContainer.id = 'all-receipts-container'; // Thêm ID để dễ dàng truy cập
        allReceiptsContainer.style.visibility = 'hidden';
        allReceiptsContainer.style.position = 'absolute';
        allReceiptsContainer.style.width = '800px'; // Đặt chiều rộng cố định
        allReceiptsContainer.style.padding = '20px'; // Thêm padding

        document.body.appendChild(allReceiptsContainer);

        let receiptsHtml = '';
        departedHistory.forEach((tx, index) => {
            const exitTime = new Date(tx.exit_time);
            const entryTime = new Date(tx.entry_time);
            const fee = tx.fee || 0;

            // Tạo URL chứa tham số cho receipt_viewer.html
            const params = new URLSearchParams({
                orgName: 'ĐOÀN TNCS HỒ CHÍ MINH PHƯỜNG BA ĐÌNH',
                orgAddress: '68 Nguyễn Thái Học, Phường Ba Đình, Thành phố Hà Nội',
                taxId: '0123456789',
                ...
                exitTimeDisplay: formatDateTime(tx.exit_time),
                duration: calculateDuration(tx.entry_time, tx.exit_time),
                feeDisplay: fee.toLocaleString('vi-VN'), feeInWords: '',
                paymentMethod: tx.payment_method
            });

            receiptsHtml += `
                <iframe src="receipt_viewer.html?${params.toString()}" style="width: 760px; height: 600px; border: none; margin-bottom: 20px; page-break-after: always;"></iframe>
            `;
        });
        ...
        const opt = {
            margin: [0, 0, 0, 0],
            filename: `TatCaBienLai_${departedHistory[0].plate}.pdf`,
            image: { type: 'jpeg', quality: 0.98 },
            html2canvas: { scale: 2, useCORS: true },
            jsPDF: { unit: 'in', format: 'a4', orientation: 'portrait' }
        };

        html2pdf().from(allReceiptsContainer).set(opt).outputPdf().then(pdf => {
    ...
    const init = async () => {
        ...
        elements.downloadAllBtn.addEventListener('click', downloadAllReceipts);
    };
